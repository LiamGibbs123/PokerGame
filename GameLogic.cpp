 ...
 * skipping turns in later rounds?? maybe to do with applying under the gun??
 * replace 9 with constant (amount of players)  /
 * maybe have class for pot
 * use list or vector so i can use bots and human classes, then can just remove and add players to list
 ...
 * card animations
 * dark screen when annoucing winner
 */
...
string GameLogic::convertCards(GCard cards[], int sizeOfArray) {
...
void GameLogic::printTable(int turn) {
    cout << endl;
    for (int i = 0; i < 9; i++) {
        cout << (i == currentPlayer ? "**" : "");
        players[i].printInfo();
    }
    cout << "\n" << "Cards on table: " << convertCards(cardsOnTable, sizeof(Card) * turn) << endl;
    cout << "Pot: $" << pot << endl;
    cout << endl;
}
    ...
    for (int i = 0; i < 9; i++) {
        if (players[currentPlayer].getInRound() &&
            ...
            if (players[currentPlayer].getIsHuman()) {
                ...
                "\nCheck (1)/Raise (2)/Fold (3)." << endl; //is convertcards working?, arguements correct?
...
void GameLogic::newRound() {
    for (int i = 0; i < PLAYER_TOTAL; i++) {
        if (players[i].getMoney() == 0 && players[i].getInGame()) {
            ...
            cout << players[i].getName() << " is out!" << endl;
        }
    }
    ...
    cout << bigBlindPlayer << "   " << smallBlindPlayer;
...
string GameLogic::determineHand(int handStrength) {
    string hand;
    switch (handStrength) {
        case 1:
            hand = "High Card";
        ...
        case 10:
            ...
            break;
        default:
            cout << "ERROR: hand strength out of range";
    }
    return hand;
}
...
void GameLogic::checkWinningHand() {
    ...
    if (playersInRound() == 1) {
        ...
        cout << players[winner].getName() << " wins the pot!!" << endl;
    }
    else {
        for (int i = 0; i < PLAYER_TOTAL; i++) {
            if (players[i].getInRound()) {
                Hand hand(cardsOnTable, players[i].getCards());
                hand.evaluate();
        ...
        cout << players[winner].getName() << " wins the pot!!\n" << "Winning hand: " <<
        determineHand(handStrengths[winner])
        << "\n";
        cout << convertCards(cardsOnTable, sizeof(cardsOnTable)) <<
...
void GameLogic::playGame() {
    ...
    cout << "Winner is " << players[winner].getName() << "! B-ANTA" << endl;
...
void GameLogic::render(SDL_Renderer *renderer) {
    ...
    string potString = "$" + to_string(pot);
    moneyFont.getTexture().setAlpha(255);
    moneyFont.render(380, 300, 16, potString, renderer);
}
